...
export default function Home() {
  ...
  return (
    <main
      className="h-full w-full flex flex-row min-h-screen items-center justify-between bg-auto bg-no-repeat bg-right-top bg-scroll backdrop-contrast-200 backdrop-saturate-200"
      style={{ backgroundImage: "url(./venom-bg.svg)" }}
    >
      <button className="w-60 h-16 flex flex-start items-center gap-4 absolute top-5 right-10 px-5 py-2 bg-gradient-to-tr from-black to-[#363636]  rounded-full translate-y-5 shadow-lg shadow-black/40 hover:shadow-xl hover:shadow-black/40 transition-all" onClick={!address?onConnectButtonClick:onDisconnectButtonClick}>
